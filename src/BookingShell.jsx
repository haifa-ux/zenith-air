...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .nav-btn { transition: background .3s ease, color .3s ease; cursor: pointer; }
    ...
    .airport-option { transition: background .2s ease; cursor: pointer; }
    .airport-option:hover { background: #F8F6F1 !important; }

    .swap-btn { transition: transform .5s cubic-bezier(.25,.46,.45,.94), border-color .3s ease; cursor:pointer; background:white; border:1px solid #E8E8E4; }
    ...
    .trip-toggle-option { position:relative; z-index:1; border-radius:100px; border:none; background:transparent; font-family:"Noto Sans",sans-serif; font-weight:400; letter-spacing:.14em; text-transform:uppercase; cursor:pointer; transition:color .35s ease; }
    ...
    .cal-day-wrap  { position:relative; width:36px; height:36px; display:flex; align-items:center; justify-content:center; }
    .cal-range-bg  { position:absolute; top:0; bottom:0; background:#F5EDD9; pointer-events:none; transition:opacity .3s ease; }
    .cal-day { position:relative; z-index:1; width:36px; height:36px; display:flex; align-items:center; justify-content:center; border-radius:50%; font-family:"Noto Sans",sans-serif; font-size:12px; font-weight:400; cursor:pointer; transition:background .25s ease, color .25s ease, transform .25s ease; border:none; background:transparent; letter-spacing:.02em; flex-shrink:0; }
    ...
    .cal-day.past { color:#CECEC8; cursor:default; pointer-events:none; }
    .cal-month-nav { background:none; border:none; cursor:pointer; padding:6px 8px; color:#9A9A94; transition:color .25s ease, transform .3s ease; display:flex; align-items:center; justify-content:center; }
    .cal-month-nav:hover { color:#1A1A1A; transform:scale(1.1); }
    .cal-month-nav:disabled { opacity:.25; cursor:default; transform:none; }

    .time-stepper-btn { background:none; border:none; cursor:pointer; padding:4px 8px; color:#9A9A94; transition:color .25s ease, transform .2s ease; display:flex; align-items:center; line-height:1; }
    ...
    .pax-count-btn { width:40px; height:40px; border-radius:50%; border:1.5px solid #C9A96E; background:transparent; display:flex; align-items:center; justify-content:center; cursor:pointer; transition:background .3s ease, border-color .3s ease, opacity .3s ease, transform .2s ease; flex-shrink:0; }
    ...
    .pref-row { display:flex; align-items:center; justify-content:space-between; padding:15px 0; border-bottom:1px solid #E8E8E4; animation: prefIn .5s cubic-bezier(.25,.46,.45,.94) both; cursor:pointer; }
    ...
    .notes-field { width:100%; border:none; background:transparent; resize:none; font-family:"Noto Sans",sans-serif; font-size:13px; font-weight:300; color:#1A1A1A; letter-spacing:.02em; line-height:1.7; padding:0; }
    .notes-field::placeholder { color:#9A9A94; opacity:.6; }
    ...
    <div style={{ width: "100%" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        ...
        <div>
          ...
          <span style={{ fontFamily: "Cormorant Garamond,serif", fontSize: 22, fontWeight: 300, color: TOKEN.muted, letterSpacing: ".06em", marginLeft: 8 }}>{viewYear}</span>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 36px)", gap: "0 4px", justifyContent: "center", marginBottom: 6 }}>
        {DAY_LABELS.map(d => <div key={d} style={{ textAlign: "center", fontFamily: "Noto Sans,sans-serif", fontSize: 9.5, fontWeight: 500, letterSpacing: ".14em", textTransform: "uppercase", color: TOKEN.muted, padding: "4px 0 10px" }}>{d}</div>)}
      </div>
      <div key={calKey} className="cal-grid" style={{ display: "grid", gridTemplateColumns: "repeat(7, 36px)", gap: "4px 4px", justifyContent: "center" }}>
        {cells.map((cell, idx) => {
          if (cell.empty) return <div key={cell.key} style={{ width: 36, height: 36 }} />;
          ...
          return (
            <div key={cell.key} className="cal-day-wrap">
              {(inRange || rangeBg) && <div className="cal-range-bg" style={{ left: rangeBg ? rangeBg.left : "0%", right: rangeBg ? rangeBg.right : "0%" }} />}
              <button className={cls} onClick={() => !past && onSelect(cell.date)} style={{ color: past ? undefined : isDep || isRet ? "#fff" : TOKEN.text }}>{cell.day}</button>
            </div>
          );
        })}
      </div>
      {isRoundTrip && <div style={{ marginTop: 14, textAlign: "center", fontFamily: "Noto Sans,sans-serif", fontSize: 10, fontWeight: 300, color: TOKEN.muted, letterSpacing: ".08em", fontStyle: "italic", minHeight: 16, transition: "opacity .4s ease", opacity: !departDate || returnDate ? 0 : 1 }}>Now select your return date</div>}
    </div>
  );
};
  ...
  return (
    <div style={{ padding: "44px 40px 40px" }}>
      <div style={{ marginBottom: 36 }}>
        <div style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 9, letterSpacing: ".3em", textTransform: "uppercase", color: TOKEN.accent, fontWeight: 500, marginBottom: 10 }}>Step 3</div>
      ...
      <div style={{ animation: "driftUp .7s cubic-bezier(.25,.46,.45,.94) .3s both" }}>
        {!isRoundTrip ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            <div style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 9, letterSpacing: ".28em", textTransform: "uppercase", color: TOKEN.muted, fontWeight: 500, textAlign: "center", marginBottom: 20 }}>Departure Time</div>
          ...
          <div key="dual" style={{ display: "flex", alignItems: "flex-start", gap: 0 }}>
            ...
            <div className="dual-time-divider" style={{ margin: "32px 16px 0" }} />
      ...
      <div style={{ minHeight: 56, marginTop: 32, paddingTop: 24, borderTop: `1px solid ${TOKEN.secondary}` }}>
        {showConfirm ? (
          <div key={confirmKey} className="confirm-line" style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            {!isRoundTrip ? (
              <p style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 12, fontWeight: 300, color: TOKEN.text, letterSpacing: ".04em", lineHeight: 1.6 }}>
                Departure set for <span style={{ fontFamily: "Cormorant Garamond,serif", fontSize: 15, fontWeight: 400 }}>{fmtLong(departDate)}</span>
                <span style={{ color: TOKEN.muted, margin: "0 6px" }}>·</span>
                <span style={{ fontFamily: "Cormorant Garamond,serif", fontSize: 15, fontWeight: 400, color: TOKEN.accent }}>{fmtTime(depHour, depMinute, depAmpm)}</span>
              </p>
            ) : (
              <p style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 12, fontWeight: 300, color: TOKEN.text, letterSpacing: ".04em", lineHeight: 1.8 }}>
                <span style={{ fontFamily: "Cormorant Garamond,serif", fontSize: 15, fontWeight: 400 }}>{fromCity}</span>
                <span style={{ color: TOKEN.accent, margin: "0 7px", fontSize: 13 }}>→</span>
                <span style={{ fontFamily: "Cormorant Garamond,serif", fontSize: 15, fontWeight: 400 }}>{toCity}</span>
                <span style={{ color: TOKEN.accent, margin: "0 7px", fontSize: 13 }}>→</span>
                <span style={{ fontFamily: "Cormorant Garamond,serif", fontSize: 15, fontWeight: 400 }}>{fromCity}</span>
                <span style={{ color: TOKEN.muted, margin: "0 8px" }}>·</span>
                <span style={{ fontFamily: "Cormorant Garamond,serif", fontSize: 15, fontWeight: 400, color: TOKEN.accent }}>{fmt(departDate)}</span>
                <span style={{ color: TOKEN.muted, margin: "0 5px", fontSize: 11 }}>–</span>
                <span style={{ fontFamily: "Cormorant Garamond,serif", fontSize: 15, fontWeight: 400, color: TOKEN.accent }}>{fmt(returnDate)}</span>
              </p>
            )}
          </div>
        ) : (
...
const PassengerConfig = ({ selectedJet, onCountChange, prefs, onPrefsChange }) => {
  ...
  return (
    <div style={{ padding: "44px 40px 40px" }}>
      <div style={{ marginBottom: 36 }}>
        <div style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 9, letterSpacing: ".3em", textTransform: "uppercase", color: TOKEN.accent, fontWeight: 500, marginBottom: 10 }}>Step 4</div>
      ...
      <div style={{ animation: "driftUp .6s cubic-bezier(.25,.46,.45,.94) .1s both" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
          <div style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 9, letterSpacing: ".22em", textTransform: "uppercase", color: TOKEN.muted, fontWeight: 500 }}>Passengers</div>
          {jetData ? (
            <div className="cap-pill">
              ...
              <span style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 9, letterSpacing: ".14em", color: TOKEN.accent, fontWeight: 500, textTransform: "uppercase" }}>Max {maxPax} · {jetData.category}</span>
            </div>
          ) : <span style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 9, letterSpacing: ".12em", color: TOKEN.muted, fontStyle: "italic" }}>No aircraft selected</span>}
        </div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 32, marginBottom: 8 }}>
          <button className="pax-count-btn" onClick={decrement} disabled={atMin} aria-label="Remove passenger">
          ...
          <div style={{ textAlign: "center", minWidth: 80 }}>
            {editing
              ? <input ref={inputRef} className="pax-count-edit-input" value={draft} onChange={e => setDraft(e.target.value)} onBlur={() => applyCount(draft)} onKeyDown={handleEditKeyDown} maxLength={2} />
              : <button key={countKey} className="pax-count-display-btn pax-count-num" onClick={handleDisplayClick} title="Click to edit">{count}</button>
            }
            <div style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 9.5, letterSpacing: ".18em", textTransform: "uppercase", color: TOKEN.muted, fontWeight: 400, marginTop: 4 }}>{count === 1 ? "passenger" : "passengers"}</div>
          </div>
          <button className="pax-count-btn" onClick={increment} disabled={atMax} aria-label="Add passenger">
...
const JetCard = ({ jet, selected, onSelect, delay }) => (
  <div className={`jet-card${selected?" selected":""}`} onClick={() => onSelect(jet.id)}
    style={{ display: "flex", alignItems: "center", gap: 0, background: "#FFFFFF", border: `1px solid ${selected?TOKEN.accent:TOKEN.secondary}`, borderLeft: `3px solid ${selected?TOKEN.accent:"transparent"}`, borderRadius: 2, boxShadow: selected?"0 4px 24px rgba(201,169,110,.10)":"0 1px 4px rgba(26,26,26,.04)", animation: `driftUp .7s cubic-bezier(.25,.46,.45,.94) ${delay}s both`, overflow: "hidden" }}>
    <div style={{ width: 168, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, alignSelf: "stretch", background: selected?"rgba(201,169,110,.04)":"transparent", transition: "background .4s ease" }}>
    ...
    <div style={{ flex: 1, minWidth: 0, padding: "20px 22px" }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 4 }}>
        ...
        {selected && <span style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 8.5, letterSpacing: ".18em", textTransform: "uppercase", color: TOKEN.accent, fontWeight: 500 }}>Selected</span>}
      </div>
      <p style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 11.5, fontWeight: 300, color: TOKEN.muted, letterSpacing: ".02em", lineHeight: 1.4, marginBottom: 14 }}>{jet.description}</p>
      <div style={{ display: "flex", gap: 20 }}>
        {[{ icon: <IconPax />, value: `${jet.specs.passengers} pax` },{ icon: <IconRange />, value: jet.specs.range },{ icon: <IconCabin />, value: jet.specs.cabin }].map(({ icon, value }, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 5 }}>
            {icon}
            <span style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 10, fontWeight: 400, letterSpacing: ".06em", color: selected?TOKEN.text:TOKEN.muted, transition: "color .4s ease" }}>{value}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ padding: "44px 40px 40px" }}>
      <div style={{ marginBottom: 32 }}>
        <div style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 9, letterSpacing: ".3em", textTransform: "uppercase", color: TOKEN.accent, fontWeight: 500, marginBottom: 10 }}>Step 2</div>
      ...
      <div style={{ minHeight: 52, marginTop: 24, paddingTop: 20, borderTop: `1px solid ${TOKEN.secondary}` }}>
        {selectedJetData ? (
          <div key={selectedJet} className="recommend-line" style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div>
              <p style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 12, fontWeight: 300, color: TOKEN.text, letterSpacing: ".02em", lineHeight: 1.6 }}>{selectedJetData.recommendation}</p>
              <p style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 10, fontWeight: 300, color: TOKEN.muted, letterSpacing: ".04em", marginTop: 3 }}>{selectedJetData.models}</p>
            </div>
          </div>
        ) : <p style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 11, fontWeight: 300, color: TOKEN.muted, letterSpacing: ".06em", fontStyle: "italic" }}>Select a category to see aircraft details</p>}
      </div>
    </div>
  );
};
...
const AirportField = ({ label, value, onSelect, placeholder, excludeCode }) => {
  ...
  return (
    <div style={{ flex: 1, position: "relative", paddingBottom: 1 }}>
      <div style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 9, letterSpacing: ".22em", textTransform: "uppercase", color: focused?TOKEN.accent:TOKEN.muted, fontWeight: 500, marginBottom: 10, transition: "color .3s ease" }}>{label}</div>
      <div style={{ position: "relative", paddingBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
          {value && <span style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 11, fontWeight: 500, letterSpacing: ".12em", color: TOKEN.accent, flexShrink: 0 }}>{value.code}</span>}
        ...
        {value && <div style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 11, fontWeight: 300, color: TOKEN.muted, marginTop: 4 }}>{value.name} · {value.state}</div>}
        ...
        <div className="airport-dropdown" style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#FFF", border: `1px solid ${TOKEN.secondary}`, borderRadius: 2, zIndex: 100, boxShadow: "0 8px 32px rgba(26,26,26,.08)", overflow: "hidden" }}>
          {filtered.map((a, i) => (
            <div key={a.code} className="airport-option" onMouseDown={() => { onSelect(a); setQuery(a.city); setOpen(false); setFocused(false); }}
              ...
              <span style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 10, fontWeight: 500, letterSpacing: ".12em", color: TOKEN.accent, width: 42, flexShrink: 0 }}>{a.code}</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: "Cormorant Garamond,serif", fontSize: 17, fontWeight: 400, color: TOKEN.text }}>{a.city}</div>
                <div style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 10, fontWeight: 300, color: TOKEN.muted, marginTop: 2 }}>{a.name}</div>
              </div>
              <span style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 9, letterSpacing: ".1em", textTransform: "uppercase", color: TOKEN.muted }}>{a.state}</span>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div style={{ padding: "44px 40px 40px" }}>
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 9, letterSpacing: ".3em", textTransform: "uppercase", color: TOKEN.accent, fontWeight: 500, marginBottom: 10 }}>Step 1</div>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
        ...
        <button className={`swap-btn${spinning?" spinning":""}`} onClick={handleSwap} style={{ width: 36, height: 36, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 18, padding: 0 }}>
      ...
      {from && to && (
        <div style={{ marginTop: 32, paddingTop: 24, borderTop: `1px solid ${TOKEN.secondary}`, display: "flex", alignItems: "center", gap: 12, animation: "driftUp .6s cubic-bezier(.25,.46,.45,.94) both" }}>
          ...
          <span style={{ marginLeft: "auto", fontFamily: "Noto Sans,sans-serif", fontSize: 9, letterSpacing: ".2em", textTransform: "uppercase", color: TOKEN.muted }}>{tripType==="Round Trip"?"Round trip · Return date TBD":"Route confirmed"}</span>
        </div>
      )}
    </div>
  );
...
const ProgressRail = ({ currentStep }) => (
  <div className="zenith-progress" style={{ display: "flex", alignItems: "flex-start", width: "100%", maxWidth: 680, margin: "0 auto", padding: "0 8px" }}>
    {STEPS.map((step, i) => {
      ...
      return (
        <div key={step.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: isLast?"0 0 auto":"1" }}>
          <div style={{ display: "flex", alignItems: "center", width: "100%" }}>
            <div className="step-node" style={{ width: 28, height: 28, borderRadius: "50%", border: `1.5px solid ${isActive||isCompleted?TOKEN.accent:TOKEN.secondary}`, background: isActive||isCompleted?TOKEN.accent:"transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, zIndex: 1 }}>
              {isCompleted
                ...
                : <span style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 10, fontWeight: 400, color: isActive?TOKEN.bg:TOKEN.muted, lineHeight: 1 }}>{step.id}</span>}
            </div>
          ...
          <span className="step-label" style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 9.5, fontWeight: 400, letterSpacing: ".1em", textTransform: "uppercase", color: isActive?TOKEN.text:TOKEN.muted, opacity: isActive?1:isCompleted?0.7:0.5, marginTop: 8, whiteSpace: "nowrap" }}>{step.label}</span>
        </div>
      );
    })}
  </div>
...
const ItemRow = ({ item, qty, onChange }) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "14px 0", borderBottom: `1px solid ${TOKEN.secondary}` }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontFamily: "Noto Sans, sans-serif", fontSize: 10.5, fontWeight: 300, color: TOKEN.muted, letterSpacing: ".03em", lineHeight: 1.5, marginTop: 3 }}>{item.desc}</div>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        <QtyButton disabled={atMin} onClick={() => onChange(qty - 1)}>
        ...
        <span style={{ fontFamily: "Cormorant Garamond, serif", fontSize: 22, fontWeight: 300, color: qty > 0 ? TOKEN.accent : TOKEN.muted, width: 22, textAlign: "center", lineHeight: 1, transition: "color .3s ease" }}>{qty}</span>
        <QtyButton disabled={atMax} onClick={() => onChange(qty + 1)}>
...
const CateringModule = ({
  ...
  return (
    <div style={{ padding: "44px 40px 40px" }}>
      {/* Header */}
      <div style={{ marginBottom: 32 }}>
        <div style={{ fontFamily: "Noto Sans, sans-serif", fontSize: 9, letterSpacing: ".3em", textTransform: "uppercase", color: TOKEN.accent, fontWeight: 500, marginBottom: 10 }}>Step 5</div>
        ...
        {passengerCount && !noCatering && (
          <p style={{ fontFamily: "Noto Sans, sans-serif", fontSize: 11, fontWeight: 300, color: TOKEN.muted, letterSpacing: ".04em", marginTop: 10, transition: "opacity .4s ease" }}>
            Catering for{" "}
            <span style={{ fontFamily: "Cormorant Garamond, serif", fontSize: 14, fontWeight: 400, color: TOKEN.text }}>{passengerCount} {passengerCount === 1 ? "guest" : "guests"}</span>
          </p>
        )}
      </div>
...
const GroundTransport = ({
  ...
  return (
    <div style={{ padding: "44px 40px 40px" }}>
      ...
      <div style={{ marginBottom: 36 }}>
        <div style={{
          fontFamily: "Noto Sans, sans-serif", fontSize: 9, letterSpacing: ".3em",
...
const SummaryRow = ({ label, value, subvalue, delay }) => (
  <div style={{
    ...
    <div style={{
      ...
      fontFamily: "Noto Sans, sans-serif", fontSize: 9, fontWeight: 500,
      letterSpacing: ".22em", textTransform: "uppercase", color: TOKEN.muted,
    }}>
      {label}
    </div>
    {/* Value */}
    <div style={{ flex: 1, minWidth: 0 }}>
      <div style={{
        fontFamily: "Noto Sans, sans-serif", fontSize: 13.5, fontWeight: 300,
      ...
      {subvalue && (
        <div style={{
          fontFamily: "Noto Sans, sans-serif", fontSize: 11, fontWeight: 300,
          color: TOKEN.muted, letterSpacing: ".03em", marginTop: 3, lineHeight: 1.5,
        }}>
          {subvalue}
        </div>
      )}
...
const TripSummary = ({ from, to, tripType, selectedJet, passengerCount, cateringState, transportState }) => {
  ...
  if (confirmed) {
    return (
      <div style={{
        ...
        <p style={{
          fontFamily: "Noto Sans, sans-serif", fontSize: 12, fontWeight: 300,
          color: TOKEN.muted, letterSpacing: ".06em", lineHeight: 1.9, maxWidth: 340,
  ...
  return (
    <div style={{ padding: "56px 48px 52px" }}>
      ...
      <div style={{
        ...
        <div style={{
          fontFamily: "Noto Sans, sans-serif", fontSize: 9, letterSpacing: ".32em",
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{
            fontFamily: "Noto Sans, sans-serif", fontSize: 11, fontWeight: 300,
            color: TOKEN.muted, letterSpacing: ".08em",
          }}>
            {departureLine}
          </span>
          {jetData && <>
      ...
      <div style={{
        ...
        <p style={{
          ...
          color: TOKEN.muted, letterSpacing: ".06em", lineHeight: 1.6,
        }}>
    ...
    <>
      ...
      <div className="zenith-shell" style={{ minHeight: "100vh", background: TOKEN.bg, display: "flex", flexDirection: "column", alignItems: "center", padding: "48px 24px 64px" }}>
        <div style={{ textAlign: "center", marginBottom: 52 }}>
          <div style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 9, letterSpacing: ".4em", textTransform: "uppercase", color: TOKEN.muted, fontWeight: 400, marginBottom: 10 }}>Private Aviation</div>
          ...
          {!isFinalStep && (
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 32px", borderTop: `1px solid ${TOKEN.secondary}`, background: TOKEN.bg }}>
              <button className="nav-btn" onClick={() => canGoBack && setCurrentStep(s => s-1)} disabled={!canGoBack}
                style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 11, letterSpacing: ".15em", textTransform: "uppercase", fontWeight: 400, padding: "10px 24px", border: `1px solid ${canGoBack?TOKEN.text:TOKEN.secondary}`, background: "transparent", color: canGoBack?TOKEN.text:TOKEN.muted, borderRadius: 2, opacity: canGoBack?1:0.4 }}>
                Back
              </button>
              <span style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 10, letterSpacing: ".2em", textTransform: "uppercase", color: TOKEN.muted, fontWeight: 300 }}>{currentStep} / {STEPS.length}</span>
              <button className="nav-btn nav-btn-primary" onClick={() => canGoForward && setCurrentStep(s => s+1)} disabled={!canGoForward}
                style={{ fontFamily: "Noto Sans,sans-serif", fontSize: 11, letterSpacing: ".15em", textTransform: "uppercase", fontWeight: 400, padding: "10px 24px", border: "1px solid transparent", background: canGoForward?TOKEN.accent:TOKEN.secondary, color: canGoForward?"#FFFFFF":TOKEN.muted, borderRadius: 2, opacity: canGoForward?1:0.5 }}>
                Continue
              </button>
            </div>
          )}
        ...
        <div style={{ marginTop: 36, fontFamily: "Cormorant Garamond,serif", fontSize: 13, fontStyle: "italic", color: TOKEN.muted, letterSpacing: ".06em", opacity: 0.7 }}>Above everything.</div>
      </div>
    </>
  );
}
